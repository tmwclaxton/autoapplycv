 ...
 */
export async function runReedAutoApplyLoop(ctx, initialSession, runDraftAll, profileData = null) {
    const {
        resetWatchdog,
        ensureReedTab,
    ...
    if (!session.queue.length) {
        throw new Error('No Reed job listings found on the search page.');
    }

    await logSession('info', `Found ${session.queue.length} jobs on Reed.`);
    ...
    while ((await loadAutoApplySession())?.stats.applied < session.maxApplications) {
        ...
        if (session.currentIndex >= session.queue.length) {
            ...
            await logSession('info', 'Loading next page of Reed results…');
            session = await appendUniqueReedJobs(tabId, session);
            markWatchdogProgress(session);
