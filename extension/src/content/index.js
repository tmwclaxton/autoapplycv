...
(function AutoCVApplyContentScriptMain() {
    ...
    extensionContext()?.onContextInvalidated?.(
        teardownContentScriptOnInvalidContext,
    );
    ...
    function readEasyApplyHighlightKey() {
        ...
        try {
            if (
                typeof AutoCVApplyLinkedInAutoApply.getEasyApplyModalState ===
                'function'
            ) {
                const state =
                    AutoCVApplyLinkedInAutoApply.getEasyApplyModalState();
    ...
    function computeFormContentSignature() {
        ...
        if (typeof AutoCVApplyFormContentSignature !== 'undefined') {
            base =
                AutoCVApplyFormContentSignature.computeFormContentSignature(
                    document,
                );
        } else {
            const heading =
    ...
    function scheduleEasyApplyHighlightRefresh(reason = 'modal') {
        ...
        contentLog(
            ...
            'Scheduling Easy Apply outline refresh',
            { reason },
        );
    ...
    function scheduleEasyApplyHighlightRetry() {
        ...
        easyApplyHighlightRetryTimer = setTimeout(() => {
            easyApplyHighlightRetryTimer = null;
            scheduleEasyApplyHighlightRefresh(
    ...
    function noteEasyApplyHighlightState() {
        ...
        easyApplyHighlightRetryCount = 0;
        scheduleEasyApplyHighlightRefresh(
            wasOpen ? 'step-change' : 'modal-open',
        );
    ...
    function notifyFormContentSignatureChanged(signature) {
        if (
            window !== window.top ||
            isAutoApplyBurstActive() ||
            !ensureExtensionContextOrTeardown()
        ) {
            return;
        }
    ...
    function extractJobDescriptionFromPage() {
        if (
            typeof AutoCVApplyLinkedInAutoApply?.readJobDescriptionText ===
            'function'
        ) {
            const linkedInText =
                AutoCVApplyLinkedInAutoApply.readJobDescriptionText();
        ...
        if (
            typeof AutoCVApplyIndeedAutoApply?.readJobDescriptionText ===
            'function'
        ) {
            const indeedText =
                AutoCVApplyIndeedAutoApply.readJobDescriptionText();
        ...
        if (
            typeof AutoCVApplyTotalJobsAutoApply?.readJobDescriptionText ===
            'function'
        ) {
            const totaljobsText =
                AutoCVApplyTotalJobsAutoApply.readJobDescriptionText();
        ...
        if (
            typeof AutoCVApplyGlassdoorAutoApply?.readJobDescriptionText ===
            'function'
        ) {
            const glassdoorText =
                AutoCVApplyGlassdoorAutoApply.readJobDescriptionText();
        ...
        if (
            typeof AutoCVApplySimplyHiredAutoApply?.readJobDescriptionText ===
            'function'
        ) {
            const simplyHiredText =
                AutoCVApplySimplyHiredAutoApply.readJobDescriptionText();
        ...
        if (
            typeof AutoCVApplyReedAutoApply?.readJobDescriptionText ===
            'function'
        ) {
        ...
        if (
            typeof AutoCVApplyCvLibraryAutoApply?.readJobDescriptionText ===
            'function'
        ) {
            const cvLibraryText =
                AutoCVApplyCvLibraryAutoApply.readJobDescriptionText();
    ...
    function isResumeFileInput(input) {
        if (
            typeof AutoCVApplyFormHeuristics?.isApplicationResumeFileInput ===
            'function'
        ) {
            return AutoCVApplyFormHeuristics.isApplicationResumeFileInput(
                input,
            );
        }
    ...
    function isCoverLetterFileInput(input) {
        ...
        const labelRoot = input.closest(
            '.field-wrapper, .form-group, label, [class*="field"]',
        );
        const labelText = (labelRoot?.textContent || '')
            .replace(/\s+/g, ' ')
            .trim();
    ...
    function findResumeFileInput() {
        ...
        AutoCVApplyFormHeuristics.forEachIframeDocument((doc) => {
            ...
            if (
                ...
                fileInput =
                    AutoCVApplyFormHeuristics.findApplicationResumeFileInput(
                        doc,
                    );
            ...
            for (const entry of doc.querySelectorAll(
                '[data-field-path*="resume"], [data-field-path*="Resume"], .ashby-application-form-field-entry, [aria-labelledby="upload-label-resume"], [id="upload-label-resume"]',
            )) {
                if (!/resume|cv/i.test(entry.textContent || entry.id || '')) {
                    continue;
                }

                const candidate = entry.querySelector(
    ...
    function findCoverLetterFileInput() {
        ...
        AutoCVApplyFormHeuristics.forEachIframeDocument((doc) => {
            ...
            for (const entry of doc.querySelectorAll(
                '[data-field-path*="cover"], [aria-labelledby*="cover" i], [id*="cover-letter" i], [id*="cover_letter" i], [id*="upload-label-cover" i]',
            )) {
                if (
                    !/cover letter|cover_letter|cover-letter/i.test(
                        entry.textContent || entry.id || '',
                    )
                ) {
                    continue;
                }

                const labeledId = String(entry.id || '').replace(
                    /^upload-label-/,
                    '',
                );
                const labeledInput = labeledId
                    ? doc.getElementById(labeledId)
                    : null;
                const candidate =
                    entry.querySelector('input[type="file"]:not([disabled])') ||
                    entry
                        .closest(
                            '.file-upload, .field-wrapper, [role="group"], fieldset, label',
                        )
            ...
            fileInput =
                doc.querySelector(
                    'input[type="file"][name="candidate.coverLetterFile"]:not([disabled])',
                ) ||
                doc.querySelector(
                    'input[type="file"][id*="coverLetter" i]:not([disabled])',
                ) ||
                doc.querySelector(
                    'input[type="file"]#cover_letter:not([disabled])',
                ) ||
                doc.querySelector(
                    'input[type="file"][name*="cover" i]:not([disabled])',
                ) ||
                Array.from(
                    doc.querySelectorAll('input[type="file"]:not([disabled])'),
                ).find(isCoverLetterFileInput);
        });
    ...
    async function fillCoverLetterFileInput(job = null) {
        contentLog(
            'debug',
            'fill.cover-letter',
            'Attempting cover letter file attach',
            {},
        );
        ...
        try {
            ...
            const file = new File(
                [blob],
                result.fileName || 'cover-letter.pdf',
                {
                    type: result.mimeType || blob.type || 'application/pdf',
                },
            );
    ...
    function removeLegacyFillOverlay() {
        document
            .querySelectorAll('#autocvapply-fill-btn, #autocvapply-portal-bar')
            .forEach((element) => {
                element.remove();
            });
    }
    ...
    async function getFieldHighlightVisibility() {
        ...
        try {
            const response = await ctx.safeRuntimeSend({
    ...
    function scheduleOverlayRefresh(
        paintFieldHighlights = undefined,
        { urgent = false } = {},
    ) {
        if (!ensureExtensionContextOrTeardown()) {
            return;
        ...
        const delayMs =
            urgent || paintFieldHighlights === true
                ? 40
    ...
    async function runFieldHighlightRefresh(explicitPaintFieldHighlights) {
        ...
        try {
            const paintFieldHighlights =
                typeof explicitPaintFieldHighlights === 'boolean'
                    ? explicitPaintFieldHighlights
                    : (await getFieldHighlightVisibility())
            ...
            if (count === 0 && easyApplyOpen) {
                ...
                contentLog(
                    'debug',
                    'highlight.easy-apply',
                    'Easy Apply open but no fields yet; retrying',
                    {
                        retry: easyApplyHighlightRetryCount,
                    },
                );
    ...
    async function collectDraftContext(injectedProfile = null, options = {}) {
        ...
        if (!profileData?.profile) {
            contentLog(
                'warn',
                'snapshot.collect',
                'Profile not loaded for snapshot',
                {},
            );
        ...
        const snapshotOptions = {
            allowInteractiveOptionHarvest:
                options.allowInteractiveOptionHarvest === true,
        };
        const snapshot =
            typeof AutoCVApplyFieldInventory !== 'undefined'
                ...
                : null;
        const fields =
            typeof AutoCVApplyFieldInventory !== 'undefined' && snapshot
                ? AutoCVApplyFieldInventory.fieldsFromInventory(
                      snapshot.elements,
                  )
                : [];
    ...
    contentMessageListener = (message, sender, sendResponse) => {
        ...
        (async () => {
            const linkedInBurst =
                ...
                message.type.startsWith('LINKEDIN_');
            ...
            try {
                ...
                if (message.type === 'COLLECT_DRAFTABLE_FIELDS') {
                    sendResponse(
                        await collectDraftContext(null, {
                            allowInteractiveOptionHarvest:
                                message.allowInteractiveOptionHarvest === true,
                        }),
                    );
                ...
                if (message.type === 'BUILD_FIELD_SNAPSHOT') {
                    sendResponse(
                        await collectDraftContext(message.profilePayload, {
                            allowInteractiveOptionHarvest:
                                message.allowInteractiveOptionHarvest === true,
                        }),
                    );
                ...
                if (message.type === 'FILTER_UNFILLED_REQUIRED_FIELDS') {
                    const elements =
                        typeof AutoCVApplyFormHeuristics?.filterUnfilledRequiredSnapshotElements ===
                        'function'
                ...
                if (message.type === 'APPLY_DRAFT_BATCH') {
                    ...
                    function resolveApplyAnswer(
                        label,
                        rawAnswer,
                        fieldType = null,
                    ) {
                        if (
                            typeof AutoCVApplyAnswerNormalization !==
                            'undefined'
                        ) {
                            return AutoCVApplyAnswerNormalization.normalizeFieldAnswerForQuestion(
                                label,
                                rawAnswer,
                                {
                                    fieldType,
                    ...
                    function shouldClickRefBeforeBatchApply(answer) {
                        const fieldType = String(
                            answer?.field_type || answer?.dom?.type || '',
                        ).toLowerCase();
                    ...
                    async function applySingleAnswer(answer) {
                        if (shouldClickRefBeforeBatchApply(answer)) {
                            AutoCVApplyFieldInventory.clickRefAllFrames(
                                document,
                                answer.ref,
                            );
                        ...
                        const applyOptions = {
                            ...
                            dom: answer.dom || null,
                            data_field_path:
                                answer.data_field_path ||
                                answer.dom?.data_field_path ||
                                null,
                        };

                        const label = answer.label || '';
                        const normalizedAnswer = resolveApplyAnswer(
                            label,
                            answer.answer,
                            answer.field_type || null,
                        );
                        ...
                        if (
                            ...
                            filled =
                                await AutoCVApplyFieldInventory.applyAnswerByRefWithFallback(
                                    document,
                                    answer.ref,
                                    normalizedAnswer,
                                    applyOptions,
                        ...
                        if (!filled && label) {
                            filled =
                                await AutoCVApplyFormHeuristics.applyAnswerByLabelAllFrames(
                                    document,
                                    label,
                                    normalizedAnswer,
                                );
                        ...
                        contentLog(
                            filled ? 'info' : 'warn',
                            'apply.batch',
                            filled ? 'Field applied' : 'Field apply failed',
                            {
                                ref: answer.ref,
                                label: answer.label,
                                method,
                    ...
                    for (let index = 0; index < answers.length; index += 1) {
                        ...
                        const applyFieldTimeoutMs =
                            applyTimeoutMsForAnswer(answer);
                        ...
                        const timedOut = new Promise((resolve) => {
                            timeoutId = window.setTimeout(() => {
                                contentLog(
                                    'warn',
                                    'apply.batch',
                                    'Field apply timed out; continuing batch',
                                    {
                                        ref: answer?.ref,
                                        label: answer?.label,
                ...
                if (message.type === 'BRIDGE_READ_FIELD_VALUES') {
                    if (typeof AutoCVApplyFormHeuristics === 'undefined') {
                        sendResponse({
                            success: false,
                            error: 'Form heuristics unavailable.',
                        });
                    ...
                    const controls =
                        AutoCVApplyFormHeuristics.collectReadableFieldValueControlsAllFrames();
                    sendResponse(
                        AutoCVApplyFormHeuristics.summarizeReadableFieldValueControls(
                            controls,
                ...
                if (
                    message.type === 'SCAN_FORM_VALIDATION' ||
                    message.type === 'BRIDGE_SCAN_FORM_VALIDATION'
                ) {
                    ...
                    const triggerValidation =
                        message.triggerValidation !== false;
                    const state = triggerValidation
                        ? await AutoCVApplyFormValidation.scanFormValidationStateWithTrigger(
                              document,
                              {
                        ...
                        : AutoCVApplyFormValidation.scanFormValidationState(
                              document,
                          );
                ...
                if (message.type === 'VALIDATE_BLOCKED_FIELD') {
                    ...
                    const result =
                        AutoCVApplyFormValidation.validateBlockedField(
                            document,
                            {
                                ref: message.ref || null,
                                label: message.label || null,
                                question:
                                    message.question || message.label || null,
                ...
                if (message.type === 'BRIDGE_CLICK_TEXT') {
                    const needle = String(message.text || '')
                        .replace(/\s+/g, ' ')
                        .trim()
                        .toLowerCase();
                    let clicked = false;
                    ...
                    if (!needle) {
                        ...
                        // Prefer exact label matches so "Send" does not hit "Send me a link".
                ...
                if (
                    message.type === 'APPLY_DRAFT_ANSWER' ||
                    message.type === 'APPLY_ANSWER_TO_FIELD'
                ) {
                    let filled = false;
                    let method = null;
                    const label = message.label || '';
                    const normalizedAnswer =
                        typeof AutoCVApplyAnswerNormalization !== 'undefined'
                            ? AutoCVApplyAnswerNormalization.normalizeFieldAnswerForQuestion(
                                  label,
                                  message.answer,
                                  {
                        ...
                        filled =
                            await AutoCVApplyFieldInventory.applyAnswerByRefWithFallback(
                                ...
                                normalizedAnswer,
                                {
                                    ...
                                    dom: message.dom || null,
                                    data_field_path:
                                        message.data_field_path ||
                                        message.dom?.data_field_path ||
                                        null,
                                },
                            );
                        method = 'ref';
                ...
                if (message.type === 'FILL_COVER_LETTER') {
                    sendResponse({
                        success: await fillCoverLetterFileInput(
                            message.job || null,
                        ),
                    });
                ...
                if (message.type === 'GET_JOB_META') {
                    void (async () => {
                        if (
                            typeof AutoCVApplyLinkedInAutoApply?.prepareJobDescriptionForRead ===
                            'function'
                        ) {
                ...
                if (message.type === 'LINKEDIN_PREPARE_JOB_DESCRIPTION') {
                    if (typeof AutoCVApplyLinkedInAutoApply === 'undefined') {
                        sendResponse({
                            success: false,
                            error: 'LinkedIn auto-apply helpers unavailable.',
                        });
                ...
                if (message.type === 'LINKEDIN_WAIT_FOR_JOB_DESCRIPTION') {
                    ...
                    void AutoCVApplyLinkedInAutoApply.waitForJobDescriptionReady(
                        minLength,
                        20_000,
                    )
                        .then((result) =>
                ...
                if (message.type === 'LINKEDIN_PREPARE_JOB_SEARCH') {
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.prepareJobSearch(),
                    );
                ...
                if (message.type === 'LINKEDIN_SELECT_JOB') {
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.selectJobById(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'LINKEDIN_WAIT_FOR_JOB_DETAIL') {
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.waitForJobDetailReady(
                            message.jobId,
                        ),
                    );
                ...
                if (
                    message.type === 'LINKEDIN_CLICK_EASY_APPLY' ||
                    message.type === 'LINKEDIN_OPEN_EASY_APPLY'
                ) {
                    if (typeof AutoCVApplyLinkedInAutoApply === 'undefined') {
                        sendResponse({
                            success: false,
                    ...
                    const clickResult =
                        await AutoCVApplyLinkedInAutoApply.clickEasyApply();
                ...
                if (message.type === 'LINKEDIN_WAIT_FOR_STEP_READY') {
                    ...
                    AutoCVApplyLinkedInAutoApply.waitForEasyApplyStepReady(
                        message.timeoutMs || 20_000,
                    )
                ...
                if (message.type === 'LINKEDIN_VALIDATE_BLOCKED_FIELD') {
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.validateBlockedFieldAfterFill(
                            {
                                ref: message.ref,
                                label: message.label,
                                dom: message.dom,
                ...
                if (message.type === 'LINKEDIN_ENSURE_RESUME_STEP') {
                    if (typeof AutoCVApplyLinkedInAutoApply === 'undefined') {
                        sendResponse({
                            ...
                            errors: [
                                'LinkedIn auto-apply helpers unavailable.',
                            ],
                        });
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.prefillResumeStep(
                            profileData,
                        ),
                    );
                ...
                if (message.type === 'LINKEDIN_ADVANCE_EASY_APPLY') {
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.clickNextOrSubmit(),
                    );
                ...
                if (message.type === 'LINKEDIN_VERIFY_SUBMITTED') {
                    ...
                    sendResponse(
                        AutoCVApplyLinkedInAutoApply.verifySubmitted(),
                    );
                ...
                if (message.type === 'LINKEDIN_CLOSE_EASY_APPLY') {
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.closeEasyApplyModal({
                ...
                if (message.type === 'LINKEDIN_NEXT_SEARCH_PAGE') {
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.goToNextSearchPage(),
                    );
                ...
                if (message.type === 'LINKEDIN_SCAN_PAGE_HEALTH') {
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.scanPageHealth(
                            message.options || {},
                        ),
                    );
                ...
                if (message.type === 'LINKEDIN_ACCEPT_COOKIE_CONSENT') {
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.acceptCookieConsent(),
                    );
                ...
                if (message.type === 'LINKEDIN_DISMISS_BLOCKING_MODAL') {
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.dismissBlockingModal(),
                    );
                ...
                if (message.type === 'LINKEDIN_DISMISS_SAVE_DIALOG') {
                    ...
                    sendResponse(
                        await AutoCVApplyLinkedInAutoApply.dismissSaveApplicationDialog(),
                    );
                ...
                if (message.type === 'LINKEDIN_EXPORT_EASY_APPLY_MODAL') {
                    ...
                    sendResponse(
                        AutoCVApplyLinkedInAutoApply.exportEasyApplyModalDebug(),
                    );
                ...
                if (message.type === 'INDEED_PREPARE_JOB_SEARCH') {
                    ...
                    sendResponse(
                        await AutoCVApplyIndeedAutoApply.prepareJobSearch(),
                    );
                ...
                if (message.type === 'INDEED_PREPARE_JOB_VIEW') {
                    ...
                    sendResponse(
                        await AutoCVApplyIndeedAutoApply.prepareJobView({
                            force: message.force === true,
                ...
                if (message.type === 'INDEED_COLLECT_JOB_CARDS') {
                    ...
                    const health =
                        await AutoCVApplyIndeedAutoApply.scanPageHealth();
                ...
                if (message.type === 'INDEED_SELECT_JOB') {
                    ...
                    sendResponse(
                        await AutoCVApplyIndeedAutoApply.selectJobById(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'INDEED_WAIT_FOR_JOB_DETAIL') {
                    ...
                    sendResponse(
                        await AutoCVApplyIndeedAutoApply.waitForJobDetailReady(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'INDEED_APPLY_STATE') {
                    ...
                    sendResponse(
                        AutoCVApplyIndeedAutoApply.getIndeedApplyState(),
                    );
                ...
                if (message.type === 'INDEED_ABANDON_APPLY') {
                    ...
                    sendResponse(
                        await AutoCVApplyIndeedAutoApply.abandonIndeedApply(),
                    );
                ...
                if (message.type === 'INDEED_CAPTCHA_PREPARE') {
                    ...
                    sendResponse(
                        await AutoCVApplyIndeedAutoApply.prepareCaptchaForSolve(),
                    );
                ...
                if (message.type === 'INDEED_OPEN_CONTACT_INFO') {
                    ...
                    sendResponse(
                        AutoCVApplyIndeedAutoApply.openIndeedContactInfoStep(),
                    );
                ...
                if (message.type === 'INDEED_FILL_AND_ADVANCE') {
                    ...
                    try {
                        sendResponse(
                            await AutoCVApplyIndeedAutoApply.clickContinueOrSubmit(),
                        );
                    } catch (error) {
                        const stopped =
                ...
                if (message.type === 'INDEED_NEXT_SEARCH_PAGE') {
                    ...
                    sendResponse(
                        await AutoCVApplyIndeedAutoApply.goToNextSearchPage(),
                    );
                ...
                if (message.type === 'INDEED_ACCEPT_COOKIE_CONSENT') {
                    ...
                    sendResponse(
                        await AutoCVApplyIndeedAutoApply.acceptCookieConsent(),
                    );
                ...
                if (message.type === 'GLASSDOOR_PREPARE_JOB_SEARCH') {
                    if (typeof AutoCVApplyGlassdoorAutoApply === 'undefined') {
                        sendResponse({
                            success: false,
                            error: 'Glassdoor auto-apply helpers unavailable.',
                        });
                    ...
                    sendResponse(
                        await AutoCVApplyGlassdoorAutoApply.prepareJobSearch({
                ...
                if (message.type === 'GLASSDOOR_PREPARE_JOB_VIEW') {
                    ...
                    sendResponse(
                        await AutoCVApplyGlassdoorAutoApply.prepareJobView({
                            light: message.light === true,
                        }),
                    );
                ...
                if (message.type === 'GLASSDOOR_SELECT_JOB') {
                    ...
                    sendResponse(
                        await AutoCVApplyGlassdoorAutoApply.selectJobById(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'GLASSDOOR_WAIT_FOR_JOB_DETAIL') {
                    ...
                    sendResponse(
                        await AutoCVApplyGlassdoorAutoApply.waitForJobDetailReady(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'GLASSDOOR_CHECK_APPLY_AVAILABILITY') {
                    ...
                    sendResponse(
                        AutoCVApplyGlassdoorAutoApply.readApplyAvailability(),
                    );
                ...
                if (message.type === 'GLASSDOOR_OPEN_APPLY') {
                    ...
                    sendResponse(
                        await AutoCVApplyGlassdoorAutoApply.clickGlassdoorApply(),
                    );
                ...
                if (message.type === 'GLASSDOOR_NEXT_SEARCH_PAGE') {
                    ...
                    sendResponse(
                        await AutoCVApplyGlassdoorAutoApply.goToNextSearchPage(),
                    );
                ...
                if (message.type === 'GLASSDOOR_SCAN_PAGE_HEALTH') {
                    ...
                    sendResponse(
                        AutoCVApplyGlassdoorAutoApply.scanPageHealth(),
                    );
                ...
                if (message.type === 'GLASSDOOR_ACCEPT_COOKIE_CONSENT') {
                    ...
                    sendResponse(
                        await AutoCVApplyGlassdoorAutoApply.acceptCookieConsent(),
                    );
                ...
                if (message.type === 'GLASSDOOR_WAIT_FOR_JOB_DESCRIPTION') {
                    ...
                    void AutoCVApplyGlassdoorAutoApply.waitForJobDescriptionReady(
                        minLength,
                        20_000,
                    )
                        .then((result) =>
                ...
                if (message.type === 'SIMPLYHIRED_PREPARE_JOB_SEARCH') {
                    if (
                        typeof AutoCVApplySimplyHiredAutoApply === 'undefined'
                    ) {
                        sendResponse({
                            success: false,
                            error: 'SimplyHired auto-apply helpers unavailable.',
                        });
                    ...
                    sendResponse(
                        await AutoCVApplySimplyHiredAutoApply.prepareJobSearch(),
                    );
                ...
                if (message.type === 'SIMPLYHIRED_PREPARE_JOB_VIEW') {
                    ...
                    sendResponse(
                        await AutoCVApplySimplyHiredAutoApply.prepareJobView({
                            light: message.light === true,
                        }),
                    );
                ...
                if (message.type === 'SIMPLYHIRED_SELECT_JOB') {
                    ...
                    sendResponse(
                        await AutoCVApplySimplyHiredAutoApply.selectJobById(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'SIMPLYHIRED_WAIT_FOR_JOB_DETAIL') {
                    ...
                    sendResponse(
                        await AutoCVApplySimplyHiredAutoApply.waitForJobDetailReady(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'SIMPLYHIRED_CHECK_APPLY_AVAILABILITY') {
                    ...
                    sendResponse(
                        AutoCVApplySimplyHiredAutoApply.readApplyAvailability(),
                    );
                ...
                if (message.type === 'SIMPLYHIRED_OPEN_APPLY') {
                    ...
                    sendResponse(
                        await AutoCVApplySimplyHiredAutoApply.clickSimplyHiredApply(),
                    );
                ...
                if (message.type === 'SIMPLYHIRED_NEXT_SEARCH_PAGE') {
                    ...
                    sendResponse(
                        await AutoCVApplySimplyHiredAutoApply.goToNextSearchPage(),
                    );
                ...
                if (message.type === 'SIMPLYHIRED_SCAN_PAGE_HEALTH') {
                    ...
                    sendResponse(
                        AutoCVApplySimplyHiredAutoApply.scanPageHealth(),
                    );
                ...
                if (message.type === 'SIMPLYHIRED_ACCEPT_COOKIE_CONSENT') {
                    ...
                    sendResponse(
                        await AutoCVApplySimplyHiredAutoApply.acceptCookieConsent(),
                    );
                ...
                if (message.type === 'SIMPLYHIRED_WAIT_FOR_JOB_DESCRIPTION') {
                    ...
                    void AutoCVApplySimplyHiredAutoApply.waitForJobDescriptionReady(
                        minLength,
                        20_000,
                    )
                        .then((result) =>
                ...
                if (message.type === 'TOTALJOBS_PREPARE_JOB_SEARCH') {
                    if (typeof AutoCVApplyTotalJobsAutoApply === 'undefined') {
                        sendResponse({
                            success: false,
                            error: 'Totaljobs auto-apply helpers unavailable.',
                        });
                    ...
                    sendResponse(
                        await AutoCVApplyTotalJobsAutoApply.prepareJobSearch(),
                    );
                ...
                if (message.type === 'TOTALJOBS_PREPARE_JOB_VIEW') {
                    ...
                    sendResponse(
                        await AutoCVApplyTotalJobsAutoApply.prepareJobView({
                            force: message.force === true,
                            light: message.light === true,
                        }),
                    );
                ...
                if (message.type === 'TOTALJOBS_SELECT_JOB') {
                    ...
                    sendResponse(
                        await AutoCVApplyTotalJobsAutoApply.selectJobById(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'TOTALJOBS_WAIT_FOR_JOB_DETAIL') {
                    ...
                    sendResponse(
                        await AutoCVApplyTotalJobsAutoApply.waitForJobDetailReady(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'TOTALJOBS_OPEN_APPLY') {
                    ...
                    sendResponse(
                        await AutoCVApplyTotalJobsAutoApply.clickTotalJobsApply(),
                    );
                ...
                if (message.type === 'TOTALJOBS_APPLY_STATE') {
                    ...
                    sendResponse(
                        AutoCVApplyTotalJobsAutoApply.getTotalJobsApplyState(),
                    );
                ...
                if (message.type === 'TOTALJOBS_FILL_AND_ADVANCE') {
                    ...
                    sendResponse(
                        await AutoCVApplyTotalJobsAutoApply.clickContinueOrSubmit(),
                    );
                ...
                if (message.type === 'TOTALJOBS_VERIFY_SUBMITTED') {
                    ...
                    sendResponse(
                        AutoCVApplyTotalJobsAutoApply.verifySubmitted(),
                    );
                ...
                if (message.type === 'TOTALJOBS_NEXT_SEARCH_PAGE') {
                    ...
                    sendResponse(
                        await AutoCVApplyTotalJobsAutoApply.goToNextSearchPage(),
                    );
                ...
                if (message.type === 'TOTALJOBS_SCAN_PAGE_HEALTH') {
                    ...
                    sendResponse(
                        await AutoCVApplyTotalJobsAutoApply.scanPageHealth(),
                    );
                ...
                if (message.type === 'TOTALJOBS_ACCEPT_COOKIE_CONSENT') {
                    ...
                    sendResponse(
                        await AutoCVApplyTotalJobsAutoApply.acceptCookieConsent(),
                    );
                ...
                if (message.type === 'TOTALJOBS_WAIT_FOR_JOB_DESCRIPTION') {
                    ...
                    void AutoCVApplyTotalJobsAutoApply.waitForJobDescriptionReady(
                        minLength,
                        20_000,
                    )
                        .then((result) =>
                ...
                if (message.type === 'REED_PREPARE_JOB_SEARCH') {
                    if (typeof AutoCVApplyReedAutoApply === 'undefined') {
                        sendResponse({
                            success: false,
                            error: 'Reed auto-apply helpers unavailable.',
                        });
                    ...
                    sendResponse(
                        await AutoCVApplyReedAutoApply.prepareJobSearch(),
                    );
                ...
                if (message.type === 'REED_PREPARE_JOB_VIEW') {
                    ...
                    sendResponse(
                        await AutoCVApplyReedAutoApply.prepareJobView({
                            force: message.force === true,
                            light: message.light === true,
                        }),
                    );
                ...
                if (message.type === 'REED_SELECT_JOB') {
                    ...
                    sendResponse(
                        await AutoCVApplyReedAutoApply.selectJobById(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'REED_WAIT_FOR_JOB_DETAIL') {
                    ...
                    sendResponse(
                        await AutoCVApplyReedAutoApply.waitForJobDetailReady(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'REED_OPEN_APPLY') {
                    ...
                    sendResponse(
                        await AutoCVApplyReedAutoApply.clickReedApply(),
                    );
                ...
                if (message.type === 'REED_FILL_AND_ADVANCE') {
                    ...
                    sendResponse(
                        await AutoCVApplyReedAutoApply.clickContinueOrSubmit(),
                    );
                ...
                if (message.type === 'REED_NEXT_SEARCH_PAGE') {
                    ...
                    sendResponse(
                        await AutoCVApplyReedAutoApply.goToNextSearchPage(),
                    );
                ...
                if (message.type === 'REED_SCAN_PAGE_HEALTH') {
                    ...
                    sendResponse(
                        await AutoCVApplyReedAutoApply.scanPageHealth(),
                    );
                ...
                if (message.type === 'REED_ACCEPT_COOKIE_CONSENT') {
                    ...
                    sendResponse(
                        await AutoCVApplyReedAutoApply.acceptCookieConsent(),
                    );
                ...
                if (message.type === 'REED_WAIT_FOR_JOB_DESCRIPTION') {
                    ...
                    void AutoCVApplyReedAutoApply.waitForJobDescriptionReady(
                        minLength,
                        20_000,
                    )
                        .then((result) =>
                ...
                if (message.type === 'CV_LIBRARY_PREPARE_JOB_SEARCH') {
                    if (typeof AutoCVApplyCvLibraryAutoApply === 'undefined') {
                        sendResponse({
                            success: false,
                            error: 'CV-Library auto-apply helpers unavailable.',
                        });
                    ...
                    sendResponse(
                        await AutoCVApplyCvLibraryAutoApply.prepareJobSearch(),
                    );
                ...
                if (message.type === 'CV_LIBRARY_PREPARE_JOB_VIEW') {
                    ...
                    sendResponse(
                        await AutoCVApplyCvLibraryAutoApply.prepareJobView({
                            force: message.force === true,
                            light: message.light === true,
                        }),
                    );
                ...
                if (message.type === 'CV_LIBRARY_CHECK_APPLY_AVAILABILITY') {
                    ...
                    sendResponse(
                        AutoCVApplyCvLibraryAutoApply.readApplyAvailability(),
                    );
                ...
                if (message.type === 'CV_LIBRARY_SELECT_JOB') {
                    ...
                    sendResponse(
                        await AutoCVApplyCvLibraryAutoApply.selectJobById(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'CV_LIBRARY_WAIT_FOR_JOB_DETAIL') {
                    ...
                    sendResponse(
                        await AutoCVApplyCvLibraryAutoApply.waitForJobDetailReady(
                            message.jobId,
                        ),
                    );
                ...
                if (message.type === 'CV_LIBRARY_OPEN_APPLY') {
                    ...
                    sendResponse(
                        await AutoCVApplyCvLibraryAutoApply.clickCvLibraryApply(),
                    );
                ...
                if (message.type === 'CV_LIBRARY_APPLY_STATE') {
                    ...
                    sendResponse(
                        AutoCVApplyCvLibraryAutoApply.getCvLibraryApplyState(),
                    );

                    return;
                }

                if (message.type === 'CV_LIBRARY_OPEN_COVER_LETTER') {
                    if (typeof AutoCVApplyCvLibraryAutoApply === 'undefined') {
                        sendResponse({
                            success: false,
                            error: 'CV-Library auto-apply helpers unavailable.',
                    ...
                    sendResponse(
                        await AutoCVApplyCvLibraryAutoApply.openCoverLetterEditor(),
                    );

                    return;
                }

                if (message.type === 'CV_LIBRARY_FILL_GENERATED_COVER_LETTER') {
                    if (typeof AutoCVApplyCvLibraryAutoApply === 'undefined') {
                        sendResponse({
                            success: false,
                            error: 'CV-Library auto-apply helpers unavailable.',
                    ...
                    sendResponse(
                        await AutoCVApplyCvLibraryAutoApply.fillGeneratedCoverLetter(
                            {
                                text: message.text,
                ...
                if (message.type === 'CV_LIBRARY_FILL_AND_ADVANCE') {
                    ...
                    sendResponse(
                        await AutoCVApplyCvLibraryAutoApply.clickContinueOrSubmit(),
                    );
                ...
                if (message.type === 'CV_LIBRARY_VERIFY_SUBMITTED') {
                    ...
                    sendResponse(
                        AutoCVApplyCvLibraryAutoApply.verifySubmitted(),
                    );
                ...
                if (message.type === 'CV_LIBRARY_NEXT_SEARCH_PAGE') {
                    ...
                    sendResponse(
                        await AutoCVApplyCvLibraryAutoApply.goToNextSearchPage(),
                    );
                ...
                if (message.type === 'CV_LIBRARY_SCAN_PAGE_HEALTH') {
                    ...
                    sendResponse(
                        await AutoCVApplyCvLibraryAutoApply.scanPageHealth(),
                    );
                ...
                if (message.type === 'CV_LIBRARY_ACCEPT_COOKIE_CONSENT') {
                    ...
                    sendResponse(
                        await AutoCVApplyCvLibraryAutoApply.acceptCookieConsent(),
                    );
                ...
                if (message.type === 'CV_LIBRARY_WAIT_FOR_JOB_DESCRIPTION') {
                    ...
                    void AutoCVApplyCvLibraryAutoApply.waitForJobDescriptionReady(
                        minLength,
                        20_000,
                    )
                        .then((result) =>
                ...
                if (message.type === 'REFRESH_FIELD_HIGHLIGHTS') {
                    ...
                    const paintFieldHighlights =
                        typeof message.paintFieldHighlights === 'boolean'
                            ? message.paintFieldHighlights
                            : typeof message.sidePanelOpen === 'boolean'
                ...
                if (
                    ...
                    message.type === 'AUTH_STATE_CHANGED'
                ) {
                    if (!isAutoApplyBurstActive()) {
                        const paintFieldHighlights =
                            typeof message.paintFieldHighlights === 'boolean'
    ...
    if (
        extensionContext()?.safeOnMessageAddListener(contentMessageListener) !==
        true
    ) {
        try {
    ...
    globalThis.__autocvapplyContentIsLive = () =>
        !contentScriptTornDown && isExtensionContextValid();
