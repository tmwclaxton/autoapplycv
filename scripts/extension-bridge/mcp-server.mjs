...
server.tool(
    ...
    'Send LinkedIn, Indeed, Totaljobs, Glassdoor, or Reed Auto Apply via the extension bridge.',
    {
        ...
        timingLevel: z.number().int().min(1).max(5).optional().describe('Timing level 1=Careful timing (slow) through 5=Speed (fast). Uses profile when omitted.'),
        pauseBeforeSubmit: z.boolean().optional().describe('Pause on review/submit. Uses profile application_settings when omitted.'),
        stopForCoverLetter: z.boolean().optional().describe('Pause when a cover letter is required. Uses profile when omitted.'),
        autoGenerateCoverLetter: z.boolean().optional().describe('Auto-generate cover letters. Uses profile when omitted.'),
        easyApplyOnly: z.boolean().optional().describe('Only Easy/Quick Apply jobs. Uses profile when omitted.'),
        pauseOnExternalApply: z.boolean().optional().describe('Pause on external/company-site apply. Uses profile when omitted.'),
        jobBlacklist: z.string().optional().describe('Company/phrase blacklist. Uses profile when omitted.'),
    ...
    async ({
        ...
        const result = await runCommand('start_auto_apply', {
            ...
            ...(typeof pauseBeforeSubmit === 'boolean' ? { pauseBeforeSubmit } : {}),
            ...(typeof stopForCoverLetter === 'boolean' ? { stopForCoverLetter } : {}),
            ...
            ...(typeof pauseOnExternalApply === 'boolean'
                ? { pauseOnExternalApply }
                : {}),
            ...(typeof jobBlacklist === 'string' ? { jobBlacklist } : {}),
