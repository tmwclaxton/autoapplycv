...
if (all) {
    const workerCount = workersArg === undefined ? undefined : Number(workersArg);
    const output = await buildAllSnapshotsParallel(manifest.scenarios, workerCount);
...
console.error('Usage: run-snapshot.mjs --id=... | --all [--workers=N] | --html=path [--page-url=] [--page-title=]');
process.exit(1);
