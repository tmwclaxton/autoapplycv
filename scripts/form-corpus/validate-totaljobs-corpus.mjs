#!/usr/bin/env node
/**
 * Deterministic validation for syn-tj-500-* Totaljobs corpus fixtures.
 */
...
function main() {
    ...
    for (const scenario of scenarios) {
        ...
        if (scenario.category?.startsWith('totaljobs-application-')
            && !['totaljobs-application-success', 'totaljobs-application-review'].includes(scenario.category)) {
        ...
        if (scenario.category === 'totaljobs-application-review' && fieldCount < 3) {
            failures.push(`${scenario.id}: review step has only ${fieldCount} fields`);
        }

        if (!scenario.page_url?.includes('totaljobs.com')) {
            failures.push(`${scenario.id}: page_url should be on totaljobs.com`);
        }
    }
