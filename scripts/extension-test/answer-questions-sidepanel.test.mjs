...
test('Answer Questions on Page lives in always-visible auth chrome', () => {
    ...
    const buttonLabel = sidepanelHtml.indexOf('Answer Questions on Page');
...
test('sidepanel wires Answer Questions on Page to START_DRAFT_ALL', () => {
    assert.match(sidepanelJs, /START_DRAFT_ALL/);
    assert.match(sidepanelJs, /answer-questions-btn/);
    assert.match(sidepanelJs, /ANSWER_QUESTIONS_LABEL\s*=\s*'Answer Questions on Page'/);
});
