...
/** Full country list with dial codes (intl-tel-input compatible). */
...
const PREFERRED_ISO2_BY_DIAL_CODE: Partial<Record<string, string>> = {
    '+1': 'US',
    '+44': 'GB',
    '+61': 'AU',
    '+358': 'FI',
...
export function resolvePhoneCountryIso2(dialCode: string): string {
    const normalized = normalizeDialCode(dialCode);
    const preferredIso2 = PREFERRED_ISO2_BY_DIAL_CODE[normalized];

    if (preferredIso2) {
        return preferredIso2;
    }

    return (
        PHONE_COUNTRIES.find((country) => country.dialCode === normalized)
            ?.iso2 ?? 'GB'
    );
}
