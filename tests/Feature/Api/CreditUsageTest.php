...
class CreditUsageTest extends TestCase
{
    ...
    public function test_credit_usage_requires_a_count(): void
    {
        $user = User::factory()->create();
        CvProfile::factory()->for($user)->create();
        $token = $user->createToken('extension')->plainTextToken;
    ...
    public function test_credit_usage_returns_402_when_requested_count_exceeds_remaining_allowance(): void
    {
        ...
        $this->withToken($token)
            ->postJson('/api/credits', ['count' => 5])
            ->assertStatus(402)
            ->assertJsonPath('success', false)
            ->assertJsonPath('subscription.credits_remaining', 2);
    }
    ...
    public function test_credit_usage_returns_402_when_monthly_limit_is_reached(): void
    {
        ...
        $this->withToken($token)
            ->postJson('/api/credits', ['count' => 1])
            ->assertStatus(402)
            ->assertJsonPath('success', false)
            ->assertJsonPath('subscription.can_use_credits', false);
    }
}
